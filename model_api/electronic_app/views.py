...
def docs(request, add_info: dict):
    ...
    tmp = DocumentSet.objects[0].filter(doc_set_id=ea.doc_set_id, producer_id=add_info['user_id'])
    if ea.consumer_id is not None and ea.consumer_id != add_info['user_id'] and len(tmp) != 1:
...
def doc(request, add_info: dict):
    ...
    tmp = DocumentSet.objects.filter(doc_set_id=ea.doc_set_id, producer_id=add_info['user_id'])
    if ea.consumer_id is not None and ea.consumer_id != add_info['user_id'] and len(tmp) != 1:
        raise Exception('Нет доступа для просмотра')
